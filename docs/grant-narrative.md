...
HTM and GRC are undertaking the MortgageOS tokenization project through their joint venture. Three operating models use one codebase: servicer of record for HTM's own clients under its California licences; servicing-operations and technology contractor to a bank that is the official servicer; and shareholder in a licensed Philippine bank (HTM intends a 40 % position in a small provincial bank near Clark International Airport, with a partner invited for up to 20 %). The Manila team executes servicing tasks under dual control in all three and never holds signing keys. GRC's participation in the project does not transfer or extend HTM's licences.
