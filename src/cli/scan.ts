 ...
 * and if the stack contains a servicing statement, emit the payment-application inputs.
 */
...
import { applicationInputs, compareToLoan, extractServicingStatement } from '../scan/compare.js';
...
const pay = applicationInputs(statement, loan);
const report = {
  input, pages: pages.map((p) => ({ page: p.page, confidence: p.confidence, chars: p.text.length })),
  extracted: fields, matches, servicing_statement: statement, application: pay,
};
...
console.log(`application gates ${JSON.stringify(pay.gates)} -> ${pay.ok ? 'READY' : 'BLOCKED'}`);
