...
const DOCS: Array<[string, string, number]> = [
  ...
  ['02-fha-model-note.json', 'PROMISSORY_NOTE', 2],
  ['03-fha-deed-of-trust-idaho.json', 'DEED_OF_TRUST', 3],
...
export function seedSql(): string {
  ...
  rec('principal_interest', s.principal_and_interest, 'FHA model note s.3; CD p.1 Loan Terms');
  ...
  rec('mortgage_insurance', s.fha_mip, 'CD p.1 Projected Payments (FHA MIP); remitted monthly to HUD from its own payable');
  ...
  out.push(`insert into impound_accounts (impound_id, company_id, loan_id, kind, monthly_amount, annual_total, payee_name, payee_reference, balance) values
    ...
    ('9d1f4c3e-2026-4b1a-8e7f-00000000a7a2', ${C}, ${L}, 'insurance', ${s.hazard_insurance_impound}, ${parties.hazard_insurance_carrier.annual_premium}, ${q(parties.hazard_insurance_carrier.name)}, ${q('Policy ' + parties.hazard_insurance_carrier.policy_number)}, ${c.initial_escrow_deposit * 0.4});`);
  ...
  out.push(`insert into impound_disbursements (company_id, impound_id, due_date, description, amount) values (${C}, '9d1f4c3e-2026-4b1a-8e7f-00000000a7a2', ${q(`${y0 + 1}-${parties.hazard_insurance_carrier.renewal}`)}, 'Hazard policy renewal', ${parties.hazard_insurance_carrier.annual_premium});`);
  ...
  for (let n = 1; n <= loan.loan.term_months; n++) {
    ...
    out.push(`insert into servicing_payments (company_id, loan_id, period_no, due_date, amount_due, principal_part, interest_part, pi_part, tax_part, insurance_part, escrow_part, status, received_at, amount_received)
       values (${C}, ${L}, ${n}, ${q(due)}, ${s.monthly_total_sweep}, ${principal}, ${interest}, ${s.principal_and_interest}, ${s.property_tax_impound}, ${s.hazard_insurance_impound + s.fha_mip}, ${s.property_tax_impound + s.hazard_insurance_impound}, ${q(status)}, ${n === 1 ? q(due + 'T17:05:00Z') : 'null'}, ${n === 1 ? s.monthly_total_sweep : 'null'});`);
  }
  out.push('commit;');
  return out.join('\n');
}
