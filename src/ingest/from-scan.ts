import { round2, round4 } from '../domain/loan-math.js';
...
export function classifyPage(text: string): DocKind {
  ...
  if (/PROMISE TO PAY|FHA FIXED RATE NOTE|MULTISTATE FIXED RATE NOTE|UNIFORM SECURED NOTE/.test(t)) return 'note';
...
export function buildCanonicalFromScan(pageTexts: string[]): ScanBuild {
  ...
  const mip = need('servicing.fha_mip', money(/Mortgage Insurance(?: \(FHA MIP [\d.]+%\))?:?\s?\$?\s?([\d,]+\.\d{2})/i, cd), 'closing_disclosure', 0);
  const tax = need('servicing.property_tax_impound', money(/Property Taxes \(escrow\):?\s?\$?\s?([\d,]+\.\d{2})/i, cd), 'closing_disclosure', 0);
  const hoi = need('servicing.hazard_insurance_impound', money(/Homeowner'?s Insurance \(escrow\):?\s?\$?\s?([\d,]+\.\d{2})/i, cd), 'closing_disclosure', 0);
  ...
  const loan: CanonicalLoan = {
    ...
    loan: {
      loan_id: loanId, loan_type: loanType, credit_purpose: 'consumer', fha_case_number: caseNo, product: `${term / 12}-Year Fixed Rate`, purpose: /Purpose Purchase/i.test(cd) ? 'Purchase' : 'Refinance',
      note_form: loanType === 'FHA' ? 'FHA Fixed Rate Note (HUD model note), Idaho' : 'Fannie Mae/Freddie Mac Multistate Fixed Rate Note', currency: 'USD',
    ...
    property: {
      ...
      apn, legal_description: legal, appraised_value: appraised ?? price, contract_sales_price: price, ltv: ltv ?? (price ? round4(baseAmt / Math.min(price, appraised ?? price)) : 0),
    },
    security_instrument: { type: 'Deed of Trust', form: /FHA/i.test(dot) ? 'FHA Idaho Deed of Trust (HUD model security instrument)' : 'Deed of Trust', lien_position: lien, trustee, recording_number: dotRec, recording_date: dotDate, recording_time: toTime(dotTime), recording_office: office ?? '' },
    ...
    servicing: { principal_and_interest: pi, property_tax_impound: tax, hazard_insurance_impound: hoi, fha_mip: mip, monthly_total_sweep: sweep },
